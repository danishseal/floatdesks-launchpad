...
| 2026-09-04 | session 01LqS83j | DeskHook pools section (DONE, absent until a hook is deployed). NOW: the LP surface. The board shows the Desk vault and the funding queue, which is not "add and remove liquidity as you please"; the real LP venues are the EIGHT graduated v4 pools on 4663 (4 meme, 4 quote) across MARIO/SLEEPY/SNOOZE/DOZE, all carrying live liquidity. Taking `app/src/lib/float/pools.ts` and `app/src/app/api/float/lp-pools/` (both NEW), plus a pools section in the liquidity board. NOTE: three of the four launches live on the SUPERSEDED CurveFunder 0xD55E56Be, which no registry key points at, so enumeration has to span both launchers.
