...
export function AssetImage({ image, name, fallback }: { image: string | null; name: string; fallback: string }) { return <div className="h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-[var(--color-border-soft)] bg-[var(--color-bg-raised)] text-[var(--color-text-muted)]"><TokenArt src={image} symbol={fallback} alt={name} className="h-full w-full object-cover text-sm" /></div>; }
