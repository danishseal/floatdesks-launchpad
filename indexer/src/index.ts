...
const CATALOG_PATH =
  ...
  fileURLToPath(new URL("../bundled/underlyings.json", import.meta.url));
...
app.get("/launch/ticker/:ticker", (req, res) => {
  res.json(tickerAvailability(req.params.ticker));
});

app.get("/launch/readiness", async (_req, res) => {
  try {
    res.json(await launchReadiness(RPC));
  } catch (e: any) {
    res.status(503).json({
      ready: false,
...
// the protocol treasury and half to the launch's selected fee receiver.
  ...
  try {
    ...
    for (const [market, meta] of Object.entries(listings)) {
      try {
        ...
        const feeReceiver = (meta as any).feeReceiver;
        const creator = new PublicKey(
          feeReceiver?.kind === "x" ||
          feeReceiver?.kind === "youtube" ||
          feeReceiver?.kind === "elitefourum"
            ? feeReceiver.escrow
            : feeReceiver?.value ?? (meta as any).launchedBy
        );
        const treasuryInstruction = await prog.methods
          .withdrawFees(new BN(half))
          .accountsPartial({
            global: globalPda,
            admin: admin.publicKey,
            market: marketPk,
            solVault: vault,
            recipient: new PublicKey(FEE_TREASURY),
          })
          .instruction();
        const creatorInstruction = await prog.methods
          .withdrawFees(new BN(fees - half))
          .accountsPartial({
            global: globalPda,
            admin: admin.publicKey,
            market: marketPk,
        ...
        await provider.sendAndConfirm(
          new Transaction().add(treasuryInstruction, creatorInstruction)
        );
